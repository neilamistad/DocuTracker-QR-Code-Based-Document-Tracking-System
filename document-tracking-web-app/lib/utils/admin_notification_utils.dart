  ...
  static Future<void> syncAdminBadgeCounts() async {
    ...
    try {
      final results = await Future.wait([
        // Password Requests
        supabase.from('password_requests').select('id').eq('is_read_admin', false),
        // Deletion Requests
        supabase.from('document_deletion_requests').select('id').eq('is_read_admin', false),
        // QR Reprint Requests
        supabase.from('qr_reprint_requests').select('id').eq('is_read_admin', false),
        // Faculty Account Requests
        supabase.from('faculty_account_registration_requests').select('faculty_id').eq('is_read_admin', false), // NEW
      ]);
